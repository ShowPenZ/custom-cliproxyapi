...
func authReauthRequired(auth *Auth) bool {
	if auth == nil {
		return false
	}
	if strings.EqualFold(strings.TrimSpace(auth.StatusMessage), "reauth_required") {
		return true
	}
	return isReauthRequiredError(auth.LastError)
}
