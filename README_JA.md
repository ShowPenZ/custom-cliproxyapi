...
- OAuthおよびAPIキーのアップストリームアカウントの実行時状態確認（`online`、`cooling`、`retry-wait`、`disabled`）
...
- `GET /v0/management/runtime-auths` でアップストリームアカウントの実行時状態を確認できます
- レスポンスにはクールダウン復帰時刻と再試行時刻が含まれ、どのアカウントがオンラインか、どれがクールダウン中かを判断できます
- HTTPSリバースプロキシ、チーム用APIキー配布、使用量集計、`cliproxy-upstream-status` の実運用例は [LOCAL_SETUP.md](LOCAL_SETUP.md) を参照してください
